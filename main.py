...
def check_bybit():
    try:
        # Берём список всех спотовых символов
        url = "https://api.bybit.com/spot/v1/symbols"
        r = requests.get(url, timeout=10)
        if r.status_code != 200:
            return "Ошибка проверки Bybit: сервер недоступен"
        
        data = r.json()
        symbols = [s["name"] for s in data.get("result", [])]

        if "DEXNETUSDT" in symbols:
            # Получаем цену, если монета есть
