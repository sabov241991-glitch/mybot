...
# --- Проверка монеты на Bybit (спот) ---
def check_bybit():
    try:
        url = "https://api.bybit.com/spot/quote/v1/ticker/price?symbol=DEXNETUSDT"
        r = requests.get(url, timeout=10)
        data = r.json()

        if "price" in data:
            return f"🚀 DEXNET появилась на Bybit! Текущая цена: {data['price']} $"
        else:
